...
const ALL_POSTS = [
  ...
  { id: 'post-recipe', author_id: 'user-2', kind: 'post', recipe_id: 'r1', caption: 'Nailed this one.', photo_path: null, video_uid: null, mood: null, restaurant_name: null, created_at: new Date().toISOString(), profiles: { display_name: 'Cook B', initials: 'CB' }, recipes: { title: 'Lemon Herb Chicken', hero_photo_path: null } },
...
function chain(table) {
  ...
  const builder = {
    ...
    then(resolve) {
      window.__CALLS__.push({ table, rangeArgs, eqArgs, notArgs, inArgs });
      let result = [];
      if (table === 'posts') {
        result = ALL_POSTS;
